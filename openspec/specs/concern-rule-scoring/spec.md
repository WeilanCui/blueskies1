...
- **THEN** the formulation's final score is reduced by `round(rule.weight × concern confidence)` and the match lists a penalty impact carrying the rule's rationale
...
#### Scenario: Recommend rule scores through coverage

- **WHEN** an active concern has a RECOMMEND rule matching the formulation
- **THEN** the rule contributes a positive boost impact and participates in the concern's coverage summary, as specified by `concern-coverage-scoring`
...
### Requirement: Concern-sourced impacts are distinguishable and explainable

Every impact in a recommendation match SHALL carry a `source` discriminator (`constraint` or `concern`); concern-sourced impacts SHALL carry the concern's slug and a reason derived from the rule's rationale.

#### Scenario: API response separates sources

- **WHEN** a match contains both a constraint penalty and a concern-rule penalty
- **THEN** the serialized impacts distinguish them via `source`, and the concern impact includes the concern slug

#### Scenario: Engine composition is injectable

- **WHEN** `RecommendationMatcher` is constructed without extra evaluators
- **THEN** it scores exactly as before (constraints only), preserving all existing tests
