...
- [x] 1.2 Declare the overridable variables with `?=`: `REGISTRY` (`direct:5000`), `PROJECT` (`blueskies`), `TAG` (`latest`), `DOCKER` (`docker`), `PLATFORM` (empty), `BUILD_ARGS` (empty), `BACKEND_TARGET` (empty), `FRONTEND_TARGET` (empty)
...
- [x] 6.3 Confirm both built images contain the expected application code and expose their documented ports
