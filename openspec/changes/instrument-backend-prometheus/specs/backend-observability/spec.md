...
The Django backend service SHALL serve a `GET /metrics` endpoint that returns Prometheus text format (`Content-Type: text/plain; version=0.0.4; charset=utf-8`) containing at minimum request-count, request-latency-histogram, response-size, and database-query counters for every registered view.
...
- **THEN** the response status is `200`, the `Content-Type` header starts with `text/plain; version=0.0.4`, and the body contains at least the metric families `django_http_requests_total_by_view_transport_method_total`, `django_http_requests_latency_seconds_by_view_method_bucket`, and `django_db_execute_total`
